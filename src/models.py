...
class TestOutputMetadata:
    __test__ = False
    def __init__(self, test_folder_path: str, page_title: str, page_description: str, repository_url: str, code_commit: str):
...
class PageGenSettings:
    def __init__(self, ref_test_metadata: TestOutputMetadata, comp_test_metadata: TestOutputMetadata, similar_percent_limit: float = 1):
        """All metadata used to generate the test results page.

        Args:
            ref_test_metadata (TestOutputMetadata): Metadata from reference test output file
            comp_test_metadata (TestOutputMetadata): Metadata from compared test output file
            similar_percent_limit (float): Maximum percentage signifying similarity.
        """
        self.ref_test_metadata = ref_test_metadata
        self.comp_test_metadata = comp_test_metadata
